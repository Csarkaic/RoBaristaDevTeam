...
for i=-1:-0.05:-3
    ...
    if projectileDetected == 1
        display('Foreign object detected by light curtain');
        break
    end
...
pointsInside = find(algebraicDist < 0.001);
...
for i = 1: size(tr,3)
    ...
    algebraicDist = GetAlgebraicDist(allPointsUpdated, centerPoint, radii);
    pointsInside = find(algebraicDist < 0.001);
    display(['There are ', num2str(size(pointsInside,1)),' points inside the ',num2str(i),'the ellipsoid']);
end
