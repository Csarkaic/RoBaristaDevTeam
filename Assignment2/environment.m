classdef environment < handle
    ...
    methods (Static)
        ...
        function kukaDance
            ...
            steps = 50;
            qOpt = [0 0 -pi/4 pi/2 -pi/2 -pi/2 0];
            startQ = kukBot.model.getpos();
            cuppa = Cups(1);
%             cupQ = cuppa.cup{1}.base;
            
%             pose = kukBot.model.base;
            qMatrix = zeros(steps, 7);
            
%             q0 = kukBot.model.ikcon(pose,qOpt);
...
%                 for trajStep = 1:size(jointTraj,1)
%                     q = jointTraj(trajStep,:);
%                     kukBot.model.animate(q);
%                     pause(0.1);
%                 end
%                 q0 = goalQ;
            end
